...
from fem_model import MeshInvariantDiffusion
import json
...
def sample_mesh(model, pos, cond, sched):
    ...
    """
    device = pos.device
    N, Co = pos.size(0), model.net[-1].out_features
...
def main():
    ...
    train_loader = DataLoader(train_ds, batch_size=4, shuffle=True,collate_fn=collate_mesh_batch)
    val_loader = DataLoader(val_ds, batch_size=4, shuffle=False,collate_fn=collate_mesh_batch)
    ...
    for epoch in range(model_param['num_epochs']):
        ...
        val_loss = eval_one_epoch_sample_mse(model, val_loader, device, sched)
        ...
        with open(f"loss_history_diffustion_ver1.json", "w", encoding="utf-8") as f:
            json.dump(loss_dict, f, indent=2)
        if improved:
            best_val = val_loss
            torch.save(model.state_dict(), "mesh_invariant_diffustion_ver1_early.pt")  # best만 저장

            with open(f"model_param_diffustion_ver1_early.json", "w", encoding="utf-8") as f:
        ...
        if early_stopping.should_stop:
            ...
            break
    torch.save(model.state_dict(), "mesh_invariant_diffustion_ver1.pt")

    with open(f"model_param_diffusion_ver1.json", "w", encoding="utf-8") as f:
        json.dump(model_param, f, indent=2) 
